...
`VAULT_STS_ENDPOINT_REGION`       | The region of the STS regional endpoint to authenticate with. If the AWS IAM auth mount uses a regional STS endpoint, then this needs to match the region of the endpoint. Defaults to using the global endpoint, or the region the lambda resides in if `AWS_STS_REGIONAL_ENDPOINTS` is set to `regional` | No | `eu-west-1`
